...
<?php
   ...
   <section id="HomeSec" >
    <?php   
    $event=$pdo->prepare("select  id_event,Nom from evenement  where type="."'"."Atelier Entretien"."'");
    $event->execute([]);
    while ($event1 = $event->fetch()) {
    ?>
       <?php if(isset($_GET["Ville"])) { ?>
            <button class="button-17"  onclick="window.location.href='../../siteEmsi/Evenement/atelier_entretien.php?id=<?=$event1['id_event'];?>&Ville=<?=$_GET['Ville'];?>'" > <?php echo $event1['Nom']; ?></button>      

      <?php  }
      else { ?>
                  <button class="button-17"  onclick="window.location.href='../../siteEmsi/Evenement/atelier_entretien.php?id=<?=$event1['id_event'];?>'" > <?php echo $event1['Nom']; ?></button>      
<?php
      }
    } ?>
