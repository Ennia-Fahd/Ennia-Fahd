...
<div  id="PathToFile" style="display:inline-flex;position: relative; top: 14%;">
       <?php if(isset($_GET["Ville"])){ ?>
        ...
        <img  src="../Image/icons8-more-than-24.png" alt=""><a href="../../SiteEmsi/Evenement/MenuCvLM.php?Ville=<?php echo $_GET["Ville"];?>">Menu des CV & Lettre de Motivation</a>
...
<img  src="../Image/icons8-more-than-24.png" alt=""><a href="../../SiteEmsi/Acceuil/acceuil_evenement1.php">Menu des Evenements</a>
        ...
        <img  src="../Image/icons8-more-than-24.png" alt=""><a href="../../SiteEmsi/Evenement/MenuCvLM.php">Menu des CV & Lettre de Motivation</a>
...
?>
<form id="animated-2" method="GET" action="Atelier_CV_LM.php">
...
<?php if(isset($_GET["Ville"])) { ?><input type="hidden" name="Ville" value="<?php echo $_GET["Ville"]; ?>"> <?php } ?>
    <input type="hidden" name="id" value="<?php echo  $idCVLM;  ?>">
...
<div class="container3" style="background:transparent;">
    <form id="survey-form" action="Atelier_CV_LM.php?id=<?=$idCVLM?>" method="Post">
      <label for="dropdown">Nom et Prénom :</label>
          ...
          <label for="dropdown">Atelier correction de CV & LM :</label>
          <?php $sem=$pdo->prepare("select * from evenement where Type='Atelier CV & LM' and id_event=$idCVLM");
          $sem->execute([]);?>
          <select class="dropdown"  id="s1" onchange="SelectRedirect()" name="SR" required>
            <option disabled value >Selectionnez un atelier de CV et lettre de motivation</option>
...
<?php if(isset($_POST['btnEnvoyerAjouterSR'])){
   $verif=$pdo->prepare("select * from cv_lm ");
   $verif->execute();
  ...
  while($test=$verif->fetch()){
   ...
   if($exist){
      echo ' Cet etudiant a déjà assisté à cet atelier de CV et lettre de motivation !';
   }
   else {
 
     $entr=$pdo->prepare("insert into cv_lm(Id_etd,Id_event,Presence) values(?,?,?)");
     $entr->execute([$_POST["NomPrenom"], $idCVLM,$_POST['Presence']]);
   }
}
...
<?php
        ...
        <label for="dropdown"> Présence :</label>
    <select class="dropdown" name="Presence" required>
      <?php $req=$pdo->prepare("select * from cv_lm where Id_etd=?");
      $req->execute([$etd]);
      $data=$req->fetch();
...
<div class="btn">
<input type="submit"  class="buttonDel" name="btnEnvoyerModifierCVLM" value="Modifier Atelier" >
</div>
...
<?php
if(isset($_POST["btnEnvoyerModifierCVLM"])){
  $verif=$pdo->prepare("select * from cv_lm ");
   $verif->execute();
  ...
  $req=$pdo->prepare("update cv_lm set Id_etd=?,Presence=? where Id_etd=?");
  ...
  $event=$nbr1[0];
  $etd=$nbr1[1];
  $suppression_csecon=$pdo->prepare("Delete from CV_LM where Id_event=? and Id_etd=?");
            ...
            <div class="col-md-12 mt-4">
                ...
                <div class="card" style="position:relative; bottom:30px;">
                    ...
                    <div class="card-body">
                        ...
                        <form action="SessionRecrutement.php" method="POST" enctype="multipart/form-data">
...
if(isset($_POST['save_excel_data']))
 ...
 <button class="button" formaction="Atelier_CV_LM.php?id=<?=$idSeminaire?>&Ville=<?=$_GET['Ville'];?>" name="Ajouter">Ajouter</button>
<?php } 
else {
  ?>
 <button class="button" formaction="Atelier_CV_LM.php?id=<?=$idSeminaire?>" name="Ajouter">Ajouter</button>

  <?php
}?>
</form>

<form action="Atelier_CV_LM.php?id=<?=$idCVLM?>"  method="Post">
...
if(isset($_GET['chercher_Sem'])){
    $sql = "select * from cv_lm sem,etudiant etd,evenement event  where event.ID_event=sem.Id_event and event.id_event= $idCVLM  and sem.Id_etd=etd.id_etd and etd.Nom_Prenom  LIKE '%".$_GET['NomPrenom']."%'  ";
    $etd_sem=$pdo->prepare($sql);
}
else {$etd_sem=$pdo->prepare("select * from cv_lm sem,etudiant etd,evenement event where event.ID_event=sem.Id_event and sem.Id_etd=etd.id_etd and event.id_event= $idCVLM");
}
$etd_sem->execute([]);
?>
        ...
        <?php if(isset($_GET["Ville"])){
          ?>
 <button class="buttonss" id="btnsupprimersem" name="supprimer_cvlm" value="supprimer" formaction="Atelier_CV_LM.php?id=<?=$idCVLM?>&Ville=<?=$_GET['Ville'];?>"  value="supprimer">Supprimer</button>
 <button class="buttonss" id="btnmodifiersem" name="modifier_cvlm" formaction="Atelier_CV_LM.php?id=<?=$idCVLM?>&Ville=<?=$_GET['Ville'];?>" value="Modifier" >Modifier</button>
      <?php  }
        else { ?>
 <button class="buttonss" id="btnsupprimersem" name="supprimer_cvlm" value="supprimer" formaction="Atelier_CV_LM.php?id=<?=$idCVLM?>"  value="supprimer">Supprimer</button>
 <button class="buttonss" id="btnmodifiersem" name="modifier_cvlm" formaction="Atelier_CV_LM.php?id=<?=$idCVLM?>" value="Modifier" >Modifier</button>
        <?php
        }
        ?>          
        ...
        <tr >
            ...
            <th>Site</th>
            <th>Groupe</th>
            <th>Nom de l'atelier de CV & lettre de motivation</th>
            <th>Présence</th>
        </tr>
        </thead>
...
?>
      ...
      <tr>
          ...
          <th>Site</th>
          <th>Groupe</th>
          <th>Nom de l'atelier CV & Lettre de motivation</th>
          <th>Présence</th>
      </tr>
...
<script >
  $("#exp").click(function(){
    ...
    filename: "CV & Lettre de motivation.xls", // do include extension
