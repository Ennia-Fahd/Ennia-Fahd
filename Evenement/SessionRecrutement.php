...
<title>Session Recrutement - Career Center</title>
<style>
...
<div  id="PathToFile" style="display:inline-flex;position: relative; top: 14%;">
       <?php if(isset($_GET["Ville"])){ ?>
        ...
        <img  src="../Image/icons8-more-than-24.png" alt=""><a href="../../SiteEmsi/Evenement/MenuSessions.php?Ville=<?php echo $_GET["Ville"];?>">Menu des Sessions de Recrutement</a>
...
<img  src="../Image/icons8-more-than-24.png" alt=""><a href="../../SiteEmsi/Acceuil/acceuil_evenement1.php">Menu des Evenements</a>
        ...
        <img  src="../Image/icons8-more-than-24.png" alt=""><a href="../../SiteEmsi/Evenement/MenuSessions.php">Menu des Sessions de Recrutement</a>

    <?php
       }
       ?>
    <?php $id=$pdo->prepare("select Nom from evenement where ID_event=?");
$id->execute([$idSession]);
$id2=$id->fetch();
$id2=$id2["Nom"];
...
?>
<form id="animated-2" method="GET" action="SessionRecrutement.php">
...
<?php if(isset($_GET["Ville"])) { ?><input type="hidden" name="Ville" value="<?php echo $_GET["Ville"]; ?>"> <?php } ?>
    <input type="hidden" name="id" value="<?php echo $idSession; ?>">
...
<div class="container3" style="background:transparent;">
    <form id="survey-form" action="SessionRecrutement.php?id=<?=$idSession?>&Ville=<?php echo $_GET['Ville']; ;?>" method="Post">
      <label for="dropdown">Nom et Prénom :</label>
          ...
          <label for="dropdown">Session de recrutement :</label>
          <?php $sem=$pdo->prepare("select * from evenement where Type='Session de recrutement' and id_event=$idSession");
          $sem->execute([]);?>
          <select class="dropdown" id="s1" onchange="SelectRedirect()" name="SR" required>
            <option disabled value >Selectionnez une session de recrutement</option>
...
<?php if(isset($_POST['btnEnvoyerAjouterSR'])){
  ...
  while($test=$verif->fetch()){
   ...
   if($exist){ ?>
     <h1>Cet etudiant a déjà assisté à cette session se recrutement !</h1>;
    
     <?php
   }
   else {
 
     $entr=$pdo->prepare("insert into sessionrecrutement(Id_etd,Id_event,Presence) values(?,?,?)");
     $entr->execute([$_POST["NomPrenom"], $idSession,$_POST['Presence']]);
   }
}
...
<?php
        ...
        <label for="dropdown"> Présence :</label>
    <select class="dropdown" name="Presence" required>
      <?php $req=$pdo->prepare("select * from sessionrecrutement where Id_etd=?");
      $req->execute([$etd]);
      $data=$req->fetch();
...
<div class="btn">
<input type="submit"  class="buttonDel" name="btnEnvoyerModifierSR" value="Modifier Étudiant" >
</div>
...
<?php
if(isset($_POST["btnEnvoyerModifierSR"])){
  $verif=$pdo->prepare("select * from sessionrecrutement ");
  $verif->execute();
 ...
 while($test=$verif->fetch()){
  ...
  if($exist){ ?>
    <h1>Cet etudiant existe déjà !</h1>;
   
    <?php
  }
  else {
  $req=$pdo->prepare("update sessionrecrutement set Id_etd=?,Presence=? where Id_etd=?");
  $req->execute([$_POST["NomPrenom"],$_POST["Presence"],$_POST["id"]]);
    ...
    $event=$nbr1[0];
    $etd=$nbr1[1];
    $suppression_csecon=$pdo->prepare("Delete from sessionrecrutement where Id_event=? and Id_etd=?");
    $suppression_csecon->execute([$event,$etd]);
      }
    }
       ...
       <div class="container" style="position:relative; top: 10%;">
        <div class="row">
            <div class="col-md-12 mt-4">
                ...
                <div class="card" style="position:relative; top:26%;">
                    <div class="card-header">
                        <h4>Importer un fichier Excel de Session de recrutement</h4>
                    </div>
                    <div class="card-body">

                        <form action="SessionRecrutement.php?id=<?=$idSession?>&Ville=<?php echo $_GET['Ville']; ;?>"  method="POST" enctype="multipart/form-data">
...
if(isset($_POST['save_excel_data']))
            ...
            foreach($data as $row)
            {
                ...
                {  
                  $etudiant= $row['0'];
                    ...
                    if($id_etd->rowCount() > 0){
                      $ID=$id_etd->fetch();
                      $ID= $ID['id_etd'];
                   
                    $SemQuery = "insert into sessionrecrutement(Id_etd,Id_event,Presence) VALUES (?,?,?,?)";
                    $result =  $pdo->prepare($SemQuery);
                    $result->execute([$ID,$idSession,$Presence]);
                    $msg = true;
                    }
                    else {
                      echo "<p> $etudiant inexistant.</p>"; 
                      break;
                    } 
 ...
 <button class="button" formaction="SessionRecrutement.php?id=<?=$idSession?>&Ville=<?=$_GET['Ville'];?>" name="Ajouter">Ajouter</button>
<?php } 
else {
  ?>
 <button class="button" formaction="Seminaire.php?id=<?=$idSeminaire?>" name="Ajouter">Ajouter</button>

  <?php
}?>
</form>

<form action="SessionRecrutement.php?id=<?=$idSession?>&Ville=<?php echo $_GET['Ville']; ;?>"  method="Post">
...
if(isset($_GET['chercher_Sem'])){
    $sql = "select * from sessionrecrutement sem,etudiant etd,evenement event  where event.ID_event=sem.Id_event and event.id_event= $idSession  and sem.Id_etd=etd.id_etd and etd.Nom_Prenom  LIKE '%".$_GET['NomPrenom']."%'  ";
    $etd_sem=$pdo->prepare($sql);
}
else {$etd_sem=$pdo->prepare("select * from sessionrecrutement sem,etudiant etd,evenement event where event.ID_event=sem.Id_event and sem.Id_etd=etd.id_etd and event.id_event= $idSession");
}
$etd_sem->execute([]);
?>
        ...
        <?php if(isset($_GET["Ville"])){
          ?>
 <button class="buttonss" id="btnsupprimersem" name="supprimer_sr" value="supprimer" formaction="SessionRecrutement.php?id=<?=$idSession?>&Ville=<?=$_GET['Ville'];?>"  value="supprimer">Supprimer</button>
 <button class="buttonss" id="btnmodifiersem" formaction="SessionRecrutement.php?id=<?=$idSession?>&Ville=<?=$_GET['Ville'];?>" name="modifier_sem" value="Modifier" >Modifier</button>
      <?php  }
        else { ?>
        <button class="buttonss" id="btnsupprimersem" name="supprimer_sr" value="supprimer" formaction="SessionRecrutement.php?id=<?=$idSession?>"  value="supprimer" >Supprimer</button>
 <button class="buttonss" id="btnmodifiersem" formaction="SessionRecrutement.php?id=<?=$idSession?>" name="modifier_sem" value="Modifier" >Modifier</button>
        <?php
        }
        ?>          
        </div>  
        ...
        <tr >
            ...
            <th>Site</th>
            <th>Groupe</th>
            <th>Nom de la session de recrutement</th>
            <th>Présence</th>
        </tr>
        </thead>
...
break;
 ...
 $(function(){
    ...
    $('.po_status').click(function () {
      ...
      if( $(".po_status").is(":checked")){
        ...
        if ($("input[type=checkbox]:checked").length == 1){
          $("#btnmodifiersem").attr('disabled','disabled');
          $("#btnsupprimersem").attr('disabled','disabled');
  ...
  else
  {
    ...
    $("#btnmodifiersem").removeAttr('disabled','disabled');
      $("#btnsupprimersem").removeAttr('disabled','disabled');
  ...
  $('.po_status').click(function () {
        ...
        if ($(this).is(':checked')) {
          if ($("input[type=checkbox]:checked").length == 1){
            ...
            $('#btnsupprimersem').removeAttr('disabled');
            $('#btnmodifiersem').removeAttr('disabled');
          }else {
            $('#btnmodifiersem').attr('disabled', true); //disable input
        }
...
<script >
  $("#exp").click(function(){
        $("#SR").table2excel({
    name: "SessionRecrutements",
    filename: "SessionRecrutements.xls", // do include extension
