...
<title>Seminaire - Career Center</title>
<style>
...
<div  id="PathToFile" style="display:inline-flex;position: relative; top: 14%;">
       <?php if(isset($_GET["Ville"])){ ?>
        ...
        <img  src="../Image/icons8-more-than-24.png" alt=""><a href="../../SiteEmsi/Evenement/MenuSeminaire.php?Ville=<?php echo $_GET["Ville"];?>">Menu des Séminaires</a>
...
<img  src="../Image/icons8-more-than-24.png" alt=""><a href="../../SiteEmsi/Acceuil/acceuil_evenement1.php">Menu des Evenements</a>
        ...
        <img  src="../Image/icons8-more-than-24.png" alt=""><a href="../../SiteEmsi/Evenement/MenuSeminaire.php">Menu des Séminaires</a>

    <?php
       }
       ?>
    <?php $id=$pdo->prepare("select Nom from evenement where ID_event=?");
$id->execute([$idSeminaire]);
$id2=$id->fetch();
$id2=$id2["Nom"];
...
?>
<form id="animated-2" method="GET" action="Seminaire.php">
...
<?php if(isset($_GET["Ville"])) { ?><input type="hidden" name="Ville" value="<?php echo $_GET["Ville"]; ?>"> <?php } ?>
    <input type="hidden" name="id" value="<?php echo  $idSeminaire; ?>">
...
<div class="container3" style="background:transparent;">
    <form id="survey-form" action="Seminaire.php?id=<?=$idSeminaire?>" method="Post">
      <label for="dropdown">Nom et Prénom :</label>
          ...
          <?php $sem=$pdo->prepare("select * from evenement where Type='Seminaire' and id_event=$idSeminaire");
          $sem->execute([]);?>
...
<?php if(isset($_POST['btnEnvoyerAjouterSR'])){
  ...
  while($test=$verif->fetch()){
   ...
   if($exist){
      echo ' Cet etudiant a déjà assisté à ce seminaire !';
   }
   else {
 
     $entr=$pdo->prepare("insert into seminaire(Id_etd,Id_event,Presence) values(?,?,?)");
     $entr->execute([$_POST["NomPrenom"], $idSeminaire,$_POST['Presence']]);
   }
}
...
if(isset($_POST['modifier_sem'])){?>
  ...
  <?php
          ...
          <label for="dropdown"> Présence :</label>
      <select class="dropdown" name="Presence" required>
        <?php $req=$pdo->prepare("select * from seminaire where Id_etd=?");
        $req->execute([$etd]);
        $data=$req->fetch();
  ...
  <div class="btn">
  <input type="submit"  class="buttonDel" name="btnEnvoyerModifierSem" value="Modifier Séminaire" >
  </div>
  ...
  <?php
  if(isset($_POST["btnEnvoyerModifierSem"])){
    $verif=$pdo->prepare("select * from seminaire");
    $verif->execute();
   ...
   while($test=$verif->fetch()){
    ...
    $req=$pdo->prepare("update seminaire set Id_etd=?,Presence=? where Id_etd=?");
  ...
  $event=$nbr1[0];
  $etd=$nbr1[1];
  $suppression_csecon=$pdo->prepare("Delete from seminaire where Id_event=? and Id_etd=?");
...
if(isset($_POST['save_excel_data']))
 ...
 <button class="button" formaction="Seminaire.php?id=<?=$idSeminaire?>&Ville=<?=$_GET['Ville'];?>" name="Ajouter">Ajouter</button>
<?php } 
else {
  ?>
...
<form action="Seminaire.php?id=<?=$idSeminaire?>"  method="Post">
...
if(isset($_GET['chercher_Sem'])){
    $sql = "select * from seminaire sem,etudiant etd,evenement event  where event.ID_event=sem.Id_event and event.id_event= $idSeminaire  and sem.Id_etd=etd.id_etd and etd.Nom_Prenom  LIKE '%".$_GET['NomPrenom']."%'  ";
    $etd_sem=$pdo->prepare($sql);
}
else {$etd_sem=$pdo->prepare("select * from seminaire sem,etudiant etd,evenement event where event.ID_event=sem.Id_event and sem.Id_etd=etd.id_etd and event.id_event= $idSeminaire");
}
$etd_sem->execute([]);
?>
        ...
        <?php if(isset($_GET["Ville"])){
          ?>
 <button class="buttonss" id="btnsupprimersem" formaction="Seminaire.php?id=<?=$idSeminaire?>&Ville=<?=$_GET['Ville'];?>" name="supprimer_sem" value="supprimer">Supprimer</button>
 <button class="buttonss" id="btnmodifiersem" formaction="Seminaire.php?id=<?=$idSeminaire?>&Ville=<?=$_GET['Ville'];?>" name="modifier_sem" value="Modifier">Modifier</button>
      <?php  }
        else { ?>
        <button class="buttonss" id="btnsupprimersem" formaction="Seminaire.php?id=<?=$idSeminaire?>" name="supprimer_sem" value="supprimer">Supprimer</button>
        <button class="buttonss" id="btnmodifiersem" formaction="Seminaire.php?id=<?=$idSeminaire?>" name="modifier_sem" value="Modifier">Modifier</button>
        <?php
        }
        ?>
        </div>  
...
<script >
  $("#exp").click(function(){
        $("#Sem").table2excel({
    name: "Séminaires",
    filename: "Séminaires.xls", // do include extension
