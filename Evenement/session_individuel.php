...
<title>Session Individuel- Career Center</title>
<style>
...
<div  id="PathToFile" style="display:inline-flex;position: relative; top: 14%;">
       <?php if(isset($_GET["Ville"])){ ?>
        ...
        <img  src="../Image/icons8-more-than-24.png" alt=""><a href="../../SiteEmsi/Evenement/MenuSessionIndividuel.php?Ville=<?php echo $_GET["Ville"];?>">Menu des Sessions Individuel</a>
...
<img  src="../Image/icons8-more-than-24.png" alt=""><a href="../../SiteEmsi/Acceuil/acceuil_evenement1.php">Menu des Evenements</a>
        ...
        <img  src="../Image/icons8-more-than-24.png" alt=""><a href="../../SiteEmsi/Evenement/MenuSessionIndividuel.php">Menu des Sessions Individuel</a>

    <?php
       }
       ?>
    <?php $id=$pdo->prepare("select Nom from evenement where ID_event=?");
$id->execute([$idSession_indiv]);
$id2=$id->fetch();
$id2=$id2["Nom"];
...
?>
<form id="animated-2" method="GET" action="session_individuel.php">
...
<?php if(isset($_GET["Ville"])) { ?><input type="hidden" name="Ville" value="<?php echo $_GET["Ville"]; ?>"> <?php } ?>
    <input type="hidden" name="id" value="<?php echo  $idSession_indiv; ?>">
...
<div class="container3" style="background:transparent;">
    <form id="survey-form" action="session_individuel.php?id=<?=$idSession_indiv?>" method="Post">
      <label for="dropdown">Nom et Prénom :</label>
          ...
          <label for="dropdown">Session individuel :</label>
          <?php $sem=$pdo->prepare("select * from evenement where Type='Atelier session individuel' and id_event=$idSession_indiv");
          $sem->execute([]);?>
          <select class="dropdown"  id="s1" onchange="SelectRedirect()" name="SR" required>
            <option disabled value >Selectionnez une session individuel</option>
...
<?php if(isset($_POST['btnEnvoyerAjouterSR'])){
   $verif=$pdo->prepare("select * from session_individuel");
   $verif->execute();
  ...
  while($test=$verif->fetch()){
   ...
   if($exist){
      echo ' Cet etudiant a déjà assisté à cette session individuel !';
   }
   else {
 
     $entr=$pdo->prepare("insert into session_individuel(Id_etd,Id_event,Presence) values(?,?,?)");
     $entr->execute([$_POST["NomPrenom"], $idSession_indiv,$_POST['Presence']]);
   }
}
...
if(isset($_POST['modifier_sess_indiv'])){?>
<div class="container3" style="background:transparent;">
    <form id="survey-form" action="session_individuel.php?id=<?=$idSession_indiv?>" method="Post">
      <label for="dropdown">Nom et Prénom :</label>
          <?php $certifetd=$pdo->prepare("select * from Etudiant");
          ...
          <?php 
             $ids_sess_indiv=implode(" ",$_POST["row-check"]);
           $nbr1=explode("-",$ids_sess_indiv);
           $event=$nbr1[0];
           $etd=$nbr1[1];
...
<?php
        ...
        <label for="dropdown"> Présence :</label>
    <select class="dropdown" name="Presence" required>
      <?php $req=$pdo->prepare("select * from session_individuel where Id_etd=?");
      $req->execute([$etd]);
      $data=$req->fetch();
...
<div class="btn">
<input type="submit"  class="buttonDel" name="btnEnvoyerModifierSess" value="Modifier Session " >
</div>
...
<?php
if(isset($_POST["btnEnvoyerModifierSess"])){
  $verif=$pdo->prepare("select * from session_individuel");
   $verif->execute();
   
   $exist=false; 
  while($test=$verif->fetch()){
      if($test["Id_etd"]==$_POST["NomPrenom"]){
          $exist=true;
          break;
      }
   }
   if($exist){
      echo ' Cet etudiant existe déjà !';
   }
   else {
  $req=$pdo->prepare("update session_individuel set Id_etd=?,Presence=? where Id_etd=?");
  $req->execute([$_POST["NomPrenom"],$_POST["Presence"],$_POST["id"]]);
    ...
    if (isset($_POST["supprimer_sess_indiv"])){
  ...
  $event=$nbr1[0];
  $etd=$nbr1[1];
  $suppression_csecon=$pdo->prepare("Delete from session_individuel where Id_event=? and Id_etd=?");
...
if(isset($_POST['save_excel_data']))
 ...
 <button class="button" formaction="session_individuel.php?id=<?=$idSession?>&Ville=<?=$_GET['Ville'];?>" name="Ajouter">Ajouter</button>
<?php } 
else {
  ?>
 <button class="button" formaction="session_individuel.php?id=<?=$idSeminaire?>" name="Ajouter">Ajouter</button>

  <?php
}?>
</form>

<form action="session_individuel.php?id=<?=$idSession_indiv?>"   method="Post">
...
if(isset($_GET['chercher_Sem'])){
  $sql = "select * from session_individuel sem,etudiant etd,evenement event  where event.ID_event=sem.Id_event and event.id_event= $idSession_indiv  and sem.Id_etd=etd.id_etd and etd.Nom_Prenom  LIKE '%".$_GET['NomPrenom']."%'  ";
  $etd_sem=$pdo->prepare($sql);
}
else {$etd_sem=$pdo->prepare("select * from session_individuel sem,etudiant etd,evenement event where event.ID_event=sem.Id_event and sem.Id_etd=etd.id_etd and event.id_event= $idSession_indiv");
}
$etd_sem->execute([]);
?>
...
<form method="POST" >
        ...
        <?php if(isset($_GET["Ville"])){
          ?>
 <button class="buttonss" id="btnsupprimersem" name="supprimer_sess_indiv" value="supprimer" formaction="session_individuel.php?id=<?=$idSession_indiv?>&Ville=<?=$_GET['Ville'];?>"  value="supprimer">Supprimer</button>
 <button class="buttonss" id="btnmodifiersem" name="modifier_sess_indiv" formaction="session_individuel.php?id=<?=$idSession_indiv?>&Ville=<?=$_GET['Ville'];?>" value="Modifier" >Modifier</button>
      <?php  }
        else { ?>
 <button class="buttonss" id="btnsupprimersem" name="supprimer_sess_indiv" value="supprimer" formaction="session_individuel.php?id=<?=$idSession_indiv?>"  value="supprimer">Supprimer</button>
 <button class="buttonss" id="btnmodifiersem" name="modifier_sess_indiv" formaction="session_individuel.php?id=<?=$idSession_indiv?>" value="Modifier" >Modifier</button>
        <?php
        }
        ?>         
        ...
        <tr >
            ...
            <th>Site</th>
            <th>Groupe</th>
            <th>Nom de la session individuel</th>
            <th>Présence</th>
        </tr>
        </thead>
...
<script >
  $("#exp").click(function(){
        $("#session").table2excel({
    name: "Session Individuel",
    filename: "Session Individuel.xls", // do include extension
